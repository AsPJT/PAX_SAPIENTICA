...
int main() {
    ...
    std::unique_ptr<paxs::SettlementSimulator<int>> simulator = std::make_unique<paxs::SettlementSimulator<int>>(map_list_path, japan_provinces_path, start_position, end_position, 10);
    simulator->init();
	simulator->run(50);
}
