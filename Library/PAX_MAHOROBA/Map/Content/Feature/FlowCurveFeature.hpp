...
#include <PAX_GRAPHICA/ColorUtility.hpp>
...
namespace paxs {
    ...
    public:
        ...
        {
            // 色情報からRGBAを取得
            // Get RGBA from color information
            if (!group_data_.color_string.empty()) {
                auto color_opt = paxg::ColorUtility::parseHexColor(group_data_.color_string);
                if (color_opt.has_value()) {
                    color_ = *color_opt;
                }
                else {
                    PAXS_WARNING("Failed to parse color string: " + group_data_.color_string + " for flow curve: " + data_.key);
                }
            }
        }
        ...
        /// @brief RGBコンポーネントを取得するヘルパー関数
        /// @brief Helper function to get RGB components
        static void getRGBComponents(const paxg::Color& color, int& r, int& g, int& b) {
        ...
        void renderSpline() const {
            ...
            // 色のRGBコンポーネントを取得
            int r, g, b;
            getRGBComponents(color_, r, g, b);

            // 背景として全体のスプライン曲線を薄く描画（指定色を使用）
            // Draw full spline curve as background (lighter color using specified color)
            paxg::Color bg_color(r, g, b, 80);  // 指定色の透明度を下げる / Reduce opacity of specified color
            ...
            constexpr FlowPath flow_paths[] = {
                ...
                {-4.0f, 0.14f * 1.5f, 0.4f},       // 左中外 / Left mid-outer
                {-2.0f, 0.28f * 1.5f, 0.8f},       // 左中内 / Left mid-inner
                { 0.0f, 0.42f * 1.5f, 0.2f},       // 中央 / Center
                { 2.0f, 0.56f * 1.5f, 0.5f},       // 右中内 / Right mid-inner
                { 4.0f, 0.70f * 1.5f, 0.9f},       // 右中外 / Right mid-outer
                { 6.0f, 0.84f * 1.5f, 0.5f}        // 右外側 / Right outer
            };
    ...
    private:
        ...
        void drawAnimatedSegment(float line_width, float phase_offset = 0.0f, float fixed_lateral_offset = 0.0f, float start_position_offset = 0.0f) const {
            ...
            if (segment_points.size() >= 2) {
                ...
                // 透明度を位相オフセットに基づいて調整（指定色を使用）
                // Adjust opacity based on phase offset (using specified color)
                const float alpha_factor = 1.0f - (phase_offset * 0.4f);  // 0.6 ~ 1.0
                const int alpha = static_cast<int>(180 * alpha_factor + 50);  // 50 ~ 230
                paxg::Color segment_color(r, g, b, alpha);  // 指定色、透明度可変 / Specified color with variable opacity
