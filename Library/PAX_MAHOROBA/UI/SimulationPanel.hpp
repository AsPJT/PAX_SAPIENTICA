...
namespace paxs {
    ...
    private:
        ...
        void simulationInit() const {
            ...
            koyomi_->jdn.setDay(static_cast<double>(SimulationConstants::getInstance(model_name)->start_julian_day)); // シミュレーション初期時の日付に設定
            koyomi_->calcDate();
        ...
        void updateSimulation() {
            ...
            // シミュレーションが初期化されている場合
                ...
                // 規定ステップ数に達したかチェック
                if (total_steps > 0 && koyomi_->steps.getDay() >= static_cast<std::size_t>(total_steps)) {
                    // 残り実行回数を減らす
    ...
    public:
        ...
        void init(
            ...
            simulation_pulldown = paxs::Pulldown(&select_language, &simulation_text, simulation_key, language_fonts, static_cast<std::uint_least8_t>(paxg::FontConfig::PULLDOWN_FONT_SIZE), static_cast<std::uint_least8_t>(paxg::FontConfig::PULLDOWN_FONT_BUFFER_THICKNESS), paxg::Vec2i{ 3000, 0 }, paxs::PulldownDisplayType::SelectedValue, false);
            pulldown_y_ = 600;
