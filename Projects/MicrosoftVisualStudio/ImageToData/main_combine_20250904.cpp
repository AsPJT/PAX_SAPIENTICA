...
constexpr int min_x = 960;
constexpr int min_y = 608;
...
int main() {
    ...
    for (int y = min_y, row0 = 0; y < max_y; ++y, ++row0) {
        for (int x = min_x, col0 = 0; x < max_x; ++x, ++col0) {
            ou(
                "C:/Users/wafuu/source/repos/HTTPDownload_0.6.131/HTTPDownload_0.6.131/App/picture/zxy_10_" + std::to_string(x) + "_" + std::to_string(y) + ".png",
                rgba.get(), row0, col0);
        }
    }
